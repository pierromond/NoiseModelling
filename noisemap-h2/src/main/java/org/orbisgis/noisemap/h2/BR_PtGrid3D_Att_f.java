...
import org.orbisgis.noisemap.core.PropagationResultPtRecord_Att_f;
import org.orbisgis.noisemap.core.jdbc.PointNoiseMap_Att_f;
 ...
 */
public class BR_PtGrid3D_Att_f extends AbstractFunction implements ScalarFunction {
    private static final int COLUMN_COUNT = 12;

    public BR_PtGrid3D_Att_f() {
    ...
    public static ResultSet noisePropagation(Connection connection, String buildings, String buildings_height_field,
        ...
        if(TableFunctionUtil.isColumnListConnection(connection)) {
            ...
            connection = SFSUtilities.wrapConnection(connection);
                    PointNoiseMap_Att_f noiseMap = new PointNoiseMap_Att_f(TableLocation.capsIdentifier(buildings, true),
    ...
    private static void feedColumns(SimpleResultSet rs) {
        rs.addColumn("IDRECEPTEUR", Types.BIGINT, 19, 20);
        rs.addColumn("IDSOURCE", Types.BIGINT, 19, 20);
        rs.addColumn("CELL_ID", Types.INTEGER, 10, 11);
        rs.addColumn("ATT", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT63", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT125", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT250", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT500", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT1000", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT2000", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT4000", Types.DOUBLE, 17, 24);
        rs.addColumn("ATT8000", Types.DOUBLE, 17, 24);


    }

    private static class PointRowSource implements SimpleRowSource {
        private Deque<PropagationResultPtRecord_Att_f> output = new ArrayDeque<PropagationResultPtRecord_Att_f>();
        private int cellI = -1;
        private int cellJ = 0;
        private PointNoiseMap_Att_f noiseMap;
        private Connection connection;

        private PointRowSource(PointNoiseMap_Att_f noiseMap, Connection connection) {
        ...
        public Object[] readRow() throws SQLException {
            ...
            // Consume cell
            PropagationResultPtRecord_Att_f record = output.pop();
            Object[] row = new Object[COLUMN_COUNT];
            row[0] = record.getReceiverRecordRow()+1;
            row[1] = record.getSourceRecordRow()+1;
            row[2] = record.getCellId();
            row[3] = record.getReceiverLvl(0);
            row[4] = record.getReceiverLvl(63);
            row[5] = record.getReceiverLvl(125);
            row[6] = record.getReceiverLvl(250);
            row[7] = record.getReceiverLvl(500);
            row[8] = record.getReceiverLvl(1000);
            row[9] = record.getReceiverLvl(2000);
            row[10] = record.getReceiverLvl(4000);
            row[11] = record.getReceiverLvl(8000);
            return row;
        }
