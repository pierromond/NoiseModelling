...
public class RailWayLWIterator implements Iterator<RailWayLWGeom> {
    private static final Logger LOGGER = LoggerFactory.getLogger("org.noise_planet.noisemodelling");
    ...
    private static String safeSchema(Connection connection) {
        try {
            ...
            return "<schema-error:" + ex.getClass().getSimpleName() + ">";
        }
    }
    ...
    private Tuple<String, Integer> resolveIntegerPrimaryKey(Connection connection, String tableName) throws SQLException {
        ...
        if (dbType == DBTypes.POSTGRESQL || dbType == DBTypes.POSTGIS) {
            ...
            if (pkColumnName == null || resolvedTable == null) {
                LOGGER.error("[RailWayLWIterator] PK lookup failed! cleanedTable={}, schemaCandidates={}, tableCandidates={}, currentSchema={}, pkColumnName={}, resolvedTable={}, resolvedSchema={}",
                    cleanedTable, schemaCandidates, Arrays.toString(tableCandidates), safeSchema(connection), 
                    pkColumnName, resolvedTable, resolvedSchema);
