...
description = 'Calculation of the Lden map from the road noise emission table (DEN format, see input details). </br> Tables must be projected in a metric coordinate system (SRID). Use "Change_SRID" WPS Block if needed. ' +
        ...
        '-  <b> Hz50,Hz63,... Hz125, Hz250, Hz500, Hz1000,Hz2000, Hz4000, Hz8000 </b> : 8 columns giving the day emission sound level for each octave band (FLOAT).'
...
def forgeCreateTable(Sql sql, String tableName, LDENConfig ldenConfig, String geomField, String tableReceiver, String tableResult, PropagationProcessPathData propagationProcessPathData) {
...
def exec(Connection connection, input) {
    ...
    environmentalData.setFreq_lvl([50,63,80,100, 125,160, 200,250,315,400 ,500,630,800, 1000,1250,1600, 2000,2500,3150, 4000,5000,6300, 8000,10000])
    environmentalData.setFreq_lvl_exact([50.0, 63.0957,80.0,100.0, 125.8925,160.0,200.0 ,251.1888,315.0,400.0, 501.1872,630.0,800.0, 1000.0,1250.0,1600.0, 1995.26231,2500.0,3150.0, 3981.07171,5000.0,6300.0 ,7943.28235,10000.0])
    ...
    if(ldenConfig.computeLDay) {
        ...
        forgeCreateTable(sql, "LDAY_GEOM", ldenConfig, geomFieldsRcv.get(0), receivers_table_name,
                ldenConfig.lDayTable,environmentalData)
    ...
    if(ldenConfig.computeLEvening) {
        ...
        forgeCreateTable(sql, "LEVENING_GEOM", ldenConfig, geomFieldsRcv.get(0), receivers_table_name,
                ldenConfig.lEveningTable,environmentalData)
    ...
    if(ldenConfig.computeLNight) {
        ...
        forgeCreateTable(sql, "LNIGHT_GEOM", ldenConfig, geomFieldsRcv.get(0), receivers_table_name,
                ldenConfig.lNightTable,environmentalData)
    ...
    if(ldenConfig.computeLDEN) {
        ...
        forgeCreateTable(sql, "LDEN_GEOM", ldenConfig, geomFieldsRcv.get(0), receivers_table_name,
                ldenConfig.lDenTable,environmentalData)
